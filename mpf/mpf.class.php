<?php
	...
	if ( !class_exists( 'MPF_FRAMEWORK' ) ) {

		/**
		 * Mpf Plugin Framework Class
		 *
		 */
		class MPF_FRAMEWORK {
			...
			function __construct( $file, $args = array() ) {
				// Default args
				$defaults = array( 'includes' => 'mpf', 'views' => 'mpf/views', 'assets' => 'mpf' );
			...
			function register_assets() {
				wp_register_style( 'mpf-plugin-framework', $this->assets( 'css', 'mpf.css' ), false, $this->version, 'all' );
				wp_register_script( 'mpf-plugin-framework-form', $this->assets( 'js', 'form.js' ), array( 'jquery' ), $this->version, false );
				wp_register_script( 'mpf-plugin-framework', $this->assets( 'js', 'mpf.js' ), array( 'mpf-plugin-framework-form' ), $this->version, false );
			}
			...
			function enqueue_assets() {
				...
				foreach ( array( 'thickbox', 'farbtastic', 'mpf-plugin-framework' ) as $style ) {
					wp_enqueue_style( $style );
				}
				foreach ( array( 'jquery', 'media-upload', 'thickbox', 'farbtastic', 'mpf-plugin-framework-form',
				                 'mpf-plugin-framework' ) as $script ) {
			 ...
			 */
			function assets( $type = 'css', $file = 'mpf.css' ) {
			...
			function render_tabs() {
				foreach ( $this->options as $option ) {
					if ( $option['type'] == 'opentab' ) {
						$active = ( isset( $active ) ) ? ' mpf-plugin-tab-inactive'
							: ' nav-tab-active mpf-plugin-tab-active';
